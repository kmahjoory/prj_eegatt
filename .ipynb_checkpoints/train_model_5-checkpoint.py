...
train_ds, test_ds = load_prepare_data_1(indx_subj=16)
...
class CNN_k1(nn.Module):
    ...
    def __init__(self, n_inpch=[5, 6, 7, 3, 5, 6, 7, 3], n_inpd2=640, n_outs=2, ks1=9, ks2=3, n_ch1=1, n_fc1=8):
        ...
        self.conv2_1 = nn.ModuleList([nn.Sequential(nn.Conv1d(1, n_ch1, kernel_size=ks2, padding=int((ks2 - 1) / 2)), nn.ReLU(), nn.AvgPool1d(kernel_size=640)) for j in range(8)])
        
        self.conv2_2 = nn.ModuleList([nn.Sequential(nn.Conv1d(2, n_ch1, kernel_size=ks2, padding=int((ks2 - 1) / 2)), nn.ReLU(), nn.AvgPool1d(kernel_size=640)) for j in range(int(8*7/2))])
...
n_epochs, lr, train_dl, test_dl = 150, 3e-3, train_dl, test_dl
