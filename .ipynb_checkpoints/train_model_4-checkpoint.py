...
class CNN_k1(nn.Module):
    ...
    def __init__(self, n_inpch=[5, 6, 7, 3, 5, 6, 7, 3], n_inpd2=640, n_outs=2, ks1=9, n_ch1=1, n_fc1=8):
        ...
        self.conv1 = nn.ModuleList([nn.Sequential(nn.Conv1d(n_inpch[j], n_ch1, kernel_size=ks1, padding=int((ks1 - 1) / 2)), nn.Relu(), nn.AvgPool1d(kernel_size=640)) for j in range(8)])
    ...
    def forward(self, x):
        #import pdb; pdb.set_trace()
        out1 = torch.cat((self.conv1[0](x[:,:5,:]), self.conv1[1](x[:,5:11,:])), 1)  # (bs, C, H,  W)      
        out2 = torch.cat((self.conv1[2](x[:,11:18,:]), self.conv1[3](x[:,18:21,:])), 1)  # (bs, C, H,  W)
        out3 = torch.cat((out1, out2), 1)
                          
        out4 = torch.cat((self.conv1[4](x[:,21:26,:]), self.conv1[5](x[:,26:32,:])), 1)  # (bs, C, H,  W)
        out5 = torch.cat((self.conv1[6](x[:,32:39,:]), self.conv1[7](x[:,39:42,:])), 1)  # (bs, C, H,  W)
