...
from eegatt_tools.models import CnnCos1layer, weight_init, CNN2d1_fc2, weight_init_cnn2d1_fc2
...
class EEGDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        eeg0 = torch.from_numpy(sig).unsqueeze(0)
...
########################################################################################################################
lr_searchspace = [1e-4, 1e-3, 1e-2]
weight_decay_searchspace = [5*1e-3]

datetime_txt = datetime.today().strftime('%Y%m%d_%H%M%S')
write_path = '/hpc/workspace/2021-0292-NeuroFlex/prj_eegatt/analyses/' + f'cv_{datetime_txt}/'
os.mkdir(write_path)
...
for lr in lr_searchspace:
    for wd in weight_decay_searchspace:
        ...
        for fold in range(nfolds):
            ...
            model = weight_init_cnn2d1_fc2(model)
