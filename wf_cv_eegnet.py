...
from eegatt_tools.models import CnnCos1layer, weight_init, Conv2d1CosFc1, weight_init_conv2d1cosfc1, EEGNet, normalize_weights_eegnet
...
lr_searchspace = [5*1e-3]
weight_decay_searchspace = [5*1e-4]
...
if save_output:
    ...
    write_path = '/hpc/workspace/2021-0292-NeuroFlex/prj_eegatt/analyses/' + f'cv_{datetime_txt}_EEGNet/'
    os.mkdir(write_path)
...
for lr in lr_searchspace:
    for wd in weight_decay_searchspace:
        ...
        model = EEGNet()
        loss_fn, acc_fn, device, verbose = nn.BCELoss(), acc_bce_1out, device, True
        ...
        for fold in range(nfolds):
            ...
            train_labels = torch.cat((torch.zeros(len(fpath_train_r)), torch.ones(len(fpath_train_l))), dim=0).to(torch.double)
            fpath_test = fpath_test_r + fpath_test_l
            test_labels = torch.cat((torch.zeros(len(fpath_test_r)), torch.ones(len(fpath_test_l))), dim=0).to(torch.double)
            ...
            # Training
            best_model, metrics = train_model(train_dl, test_dl, model, loss_fn, acc_fn, optimizer, device,
                                              weight_constraint=True, n_epochs=n_epochs)
