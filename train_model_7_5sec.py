...
train_ds, test_ds, train_filenames, test_filenames = load_prepare_data_5sec(indx_subj=5)
...
class CNN_k1(nn.Module):
    ...
    def __init__(self, n_inpch=[5, 6, 7, 3, 5, 6, 7, 3], n_inpd2=n_tps, n_outs=2, ks1=9, ks2=3, n_ch1=1, n_fc1=8):
        ...
        self.conv1 = nn.ModuleList([nn.Sequential(nn.Conv1d(n_inpch[j], n_ch1, kernel_size=ks1, padding=int((ks1 - 1) / 2)),
                                                  nn.ReLU()) for j in range(8)])
        # Connectivity matrix for 8 elements contains ((8*8)-8) = 28 left diagonal elements
        # diagonal + left off-diagonal element = 8 + 28 = 36
        self.conv2 = nn.ModuleList([nn.Sequential(nn.Conv1d(2, n_ch1, kernel_size=ks2, padding=int((ks2 - 1) / 2)),
                                                  nn.ReLU(), nn.AvgPool1d(kernel_size=n_inpd2)) for j in range(int(36))])
    ...
    def forward(self, x):
        ...
        cnt_connect = 0  # counter for 32 elements of connectivity matrix
        # 8 channel groups ->> 8*8/2= 32 elements for connectivity matrix.
        for j in range(8):
            for k in range(j, 8):
                out2.append(self.conv2[cnt_connect](out1[:, [j, k], :]))
                cnt_connect += 1
        assert cnt_connect == 36, f'Iterated over {cnt_connect} elements of connectivity matrix, instead of 32'
...
n_epochs, lr, train_dl, test_dl = 50, 1e-3, train_dl, test_dl
